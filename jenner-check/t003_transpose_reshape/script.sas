*-------------------------------------------------------------------------------------------------------*
|                                       SAS On-the-Job                                                  |
|        COVID and Female Labor Supply - State-Level Reshape via PROC TRANSPOSE (Task 3)                |
*-------------------------------------------------------------------------------------------------------*;
* Adapted from "Task 3 - State Level.sas". The state-by-quarter rate table is transposed to a long  *;
* layout and then split into separate unemployment and LFP data sets with a DATA step that keys on   *;
* the INDEX() of the transposed variable name - the exact reshape the original uses to feed its      *;
* PROC SGPANEL plots. CPS microdata are an inline weighted-cell sample for two states and quarters.   *;

title1 "SAS On-the-Job | COVID and Female Labor Supply";
title2 "State-Level Reshape | Women";
...
*-------------------------------------------------------------------------------------*
|         Collapse to state-by-quarter rates (overall, by education, by child)         |
*-------------------------------------------------------------------------------------*;
proc sql;
    ...
    select  distinct State_FIP as statefip, YearQuarter,
            ...
            sum( case when EDUC_LTD=4 and in_LF=1 and Female=1 then WTFINL else 0 end )
              / sum( case when EDUC_LTD=4 and Female=1 then WTFINL else 0 end )
                as LFP_Women_CollegeP label="College +"        format=percent9.1
    from    cps
...
*-----------------------------------------------------------------------------------------*
|              Transpose to long form to prepare for paneled plots                         |
*-----------------------------------------------------------------------------------------*;
...
*-----------------------------------------------------------------------------------------*
|        Split into unemployment and LFP data sets keyed on the variable name              |
*-----------------------------------------------------------------------------------------*;
data ue  (keep=statefip YearQuarter Group UE_Rate)
    ...
    else if index(upcase(_name_),"LFP_")=1 then do;
        LFP_Rate = col1;
...
title3 "Unemployment Rate | Long Form";
proc print data=ue noobs label; run;

title3 "Labor Force Participation Rate | Long Form";
proc print data=lfp noobs label; run;
